...
and show more generally that off-diagonal Fisher entries decay as
$O(\tanh^g(J))$ with graph girth~$g$ (verified across 71~configurations,
$C \lesssim 15$).
These results imply that sparse observer graphs select Lorentzian signature
($q = 1$) with large spectral gap margin, verified for dimensions up to
$n = 20$ (98.2\% across 111~cases).
...
decay to zero in the large-$N$ limit. Only static fixed-point rules
($N \leq 8$, shown in plateau) converge to nonzero curvature. This
empirically disconfirms the continuum limit for dynamically interesting
...
$A = F^{2}\, M^{(\mathrm{H1'})}\, F^{2} = F^{1/2}\, S\, F^{1/2}$
...
positive definite.
Define $A = F^{2}\, M^{(\mathrm{H1'})}\, F^{2}$ with
...
\caption{Eigenvalue trajectories of $A = F^{2} M^{(\mathrm{H1'})} F^{2}$
...
\begin{proof}
Define $A = F^{2}\, M^{(\mathrm{H1'})}\, F^{2}$ (well-defined
...
$A(t) = F^{2}\, M^{(\mathrm{H1'})}\, F^{2}$ corresponding to its
...
\item \textbf{Multi-eigenvalue transition sequence
  ...
  Fisher-weighted mass tensor $A = F^{2}\,M^{(\mathrm{H1'})}\,F^{2}$,
...
observer graphs, with infinite margin.
\end{corollary}
...
For an Ising model on a connected graph with girth~$g$ and uniform
coupling~$J > 0$, the Fisher information matrix satisfies:
\begin{equation}
...
\leq C \cdot \tanh^{g}(J)\,,
\end{equation}
where $C \lesssim 15$ (verified numerically across 71~configurations
with $g = 3$ to~$20$, $J = 0.1$ to~$1.0$).
...
For graphs with girth~$g < \infty$, correlations propagate through
cycles; by Dobrushin uniqueness conditions, for edges $e, f$ at
line graph distance $d(e,f)$:
$|F_{ef}| = |\operatorname{Cov}(\sigma_e, \sigma_f)|
\lesssim \operatorname{sech}^2(J) \cdot \tanh^{d(e,f)}(J)$.
The girth provides a lower bound on line graph distances; summing
over off-diagonal entries with bounded degree yields the operator
norm bound~\eqref{eq:near-diagonal-bound}.
The constant $C \approx 15$ is fitted from exact Fisher matrices on
cycle graphs $C_n$ ($n = 4, \ldots, 20$); exponential decay is
confirmed with 90\% slope agreement.
\end{proof}
...
Fitted: $C \approx 15$, slope agreement 90\% with predicted
$\tanh^g(J)$ decay (\cref{thm:near-diagonal-fisher}).}
...
\begin{enumerate}
\item \textbf{Physical origin of edge signs} (HIGH, partially addressed).
  The sign assignment $s_e \in \{+1, -1\}$ in H1$'$ is currently imposed
  rather than derived.
  Four approaches to deriving signs from causal structure were
  investigated: (1)~causal relatedness, (2)~causal flow direction,
  (3)~forward/backward path ratio, (4)~Ollivier curvature sign.
  All four fail to produce $q = 1$ directly.
  However, the spectral gap weighting (\cref{sec:spectral-gap-selection})
  provides an alternative: if the sign assignment is determined by
  \emph{any} mechanism (structural, dynamical, or random), the
  $q = 1$ outcome is overwhelmingly favored for sparse observer graphs
  by the information-geometric properties of the Fisher metric itself.
  The ``sign selection problem'' may therefore not require a specific
  causal mechanism---sparsity alone selects $q = 1$ with $96\%$--$100\%$
  success rate for tree-like topologies (\cref{tab:spectral-gap}).
...
\item \textbf{CI and exponential family statistics} (HIGH, partially
  ...
  at 55--65\% confidence.
  Preliminary computational evidence supports a \emph{causal typicality}
  conjecture: for large CI systems with small observers, the
  Kullback--Leibler divergence from MaxEnt decreases as $O(1/N)$
  with system size, analogous to quantum
  typicality~\cite{PopescuShortWinter2006}.
  If proven, this would close the deductive chain with a single
  additional epistemic principle.
...
\item \textbf{Spectral gap selection} (\cref{sec:spectral-gap-selection}):
  ...
  bound (\cref{thm:Lgap-bound}), and the $90\%$ numerical verification
  (\cref{tab:spectral-gap}) provide a conditional structural explanation
  for Lorentzian signature selection on sparse observer graphs.
  The key insight---that information-geometric properties of the Fisher
  metric itself select $q = 1$ without requiring a dynamical sign
  mechanism---appears new.

\item \textbf{Total estimated novelty}: 30--35\%.
...
The Tree Fisher Identity (\cref{thm:tree-fisher}) and spectral gap
selection mechanism (\cref{sec:spectral-gap-selection}) provide a
conditional structural explanation for why sparse observers perceive
exactly one time dimension: on tree-like observer graphs, the Fisher
metric is near-diagonal, and the spectral gap weighting selects
$q = 1$ with $96$--$100\%$ success rate on sparse topologies
($84\%$ overall; see \cref{tab:spectral-gap}).

These results are offered as tools for further development of
learning-based spacetime emergence.
The spectral gap selection mechanism suggests that the ``sign
selection problem'' may be less severe than previously thought:
sparse observers automatically select Lorentzian signature through
the information geometry of their internal interactions, without
requiring a specific dynamical mechanism for edge sign assignment.
