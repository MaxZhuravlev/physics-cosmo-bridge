...
Starting from a single axiom---causal invariance (the core symmetry of Wolfram's model)---we establish the complete structure of known physics through three uniqueness theorems plus empirical observations: (1) Einstein gravity via Lovelock's theorem, (2) natural gradient learning via Amari's theorem, (3) quantum mechanics via purification with local distinguishability emergent, plus observations on thermodynamic irreversibility and metric structure (extending Vanchurin 2017).
...
\subsection{Observation 1: Metric Identity (Vanchurin 2017)}

Vanchurin~\cite{Vanchurin2017} established the connection between Fisher information metric and spacetime Riemann metric in his ``Covariant Information Theory and Emergent Gravity.'' Both metrics satisfy the unique Einstein equations (Theorem 1) and are therefore diffeomorphic. This observation strengthens the interpretation that observer parameter space and physical spacetime are manifestations of the same underlying geometric structure.

\subsection{Observation 2: Deterministic Arrow of Time}

Combining results from both programs: On Riemannian manifold with positive definite metric $g$ (confirmed via $\kappa=0.67$):
\begin{equation}
\frac{dL}{dt} = -g^{ij}\frac{\partial L}{\partial \theta^i}\frac{\partial L}{\partial \theta^j} \leq 0
\end{equation}

Additionally, causal graphs are acyclic (from CI). This provides a deterministic (non-statistical) arrow of time from the combination of Vanchurin's learning dynamics and Wolfram's causal structure.
