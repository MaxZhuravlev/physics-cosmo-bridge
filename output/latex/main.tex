...
We prove that any positive definite diagonal matrix selects Lorentzian
signature ($q = 1$) with explicit margin, and that this dominance is
stable under perturbation (robust to off-diagonal ratio ${\sim}0.9$).
...
sizes $m \in \{2, 3, 4, 5, 6, 7, 8, 10, 12, 15\}$,
coupling strengths $J \in \{0.1, 0.2, 0.3, 0.5, 0.7, 1.0, 1.5, 2.0\}$,
and three graph topologies (tree, sparse, dense)---confirms that
Lorentzian selection ($q = 1$) achieves a win rate of exactly~$0\%$
when $M = F^2$.
In every configuration, $\beta_c = -1$ (no valid Lorentzian regime
exists) because $F^{-1/2}\,M\,F^{-1/2} = F^{-1/2}\,F^2\,F^{-1/2} = F$,
which is positive definite with no negative eigenvalues.
The obstruction is structural and independent of observer size,
coupling strength, and graph topology: it holds universally
for canonical exponential families.
This validates the necessity of the signed-edge
construction~H1$'$ (\cref{sec:H1-prime}) or Vanchurin's non-principal
square root for obtaining Lorentzian signature.
\end{remark}
...
$d_1 \geq \cdots \geq d_m > 0$ and $m \geq 3$:
\begin{equation}
...
\leq C(g, \Delta) \cdot \tanh^{g-2}(J)\,,
\end{equation}
where the effective constant $C(g, \Delta)$ grows at most linearly
in~$g$ for fixed~$\Delta$ (verified: $C \lesssim 0.7\,g$ on cycle
graphs with $g = 3$ to~$12$, $J = 0.1$ to~$1.0$, 50~configurations).
...
\max_e \sum_{f \neq e} |F_{ef}|$.
Each edge $e$ has at most $2(\Delta - 1)$ adjacent edges plus
higher-order neighbours; summing the geometric series of decaying
correlations gives a constant $C(g, \Delta)$ growing linearly in~$g$
(each additional girth step adds one more shell of non-negligible
correlations).
Division by $\|\operatorname{diag}(F)\|_{\text{op}}
= \operatorname{sech}^2(J)$ yields~\eqref{eq:near-diagonal-bound}.

\textit{Numerical verification.}
On cycle graphs $C_g$ ($g = 3, \ldots, 12$), the ratio
$C(g,\Delta) = \text{ratio}/\tanh^{g-2}(J)$ has coefficient of
...
$C(g, \Delta) \lesssim 0.7\,g$ on cycles
...
\item \textbf{Physical origin of edge signs} (MEDIUM, substantially addressed).
  ...
  Combined with the spectral gap selection mechanism
  (\cref{sec:spectral-gap-selection}), this shows that Lorentzian
  signature is a \emph{robust emergent property}: multiple independent
  mechanisms converge to $q = 1$ on sparse observer graphs.
...
\item \textbf{Spectral gap selection} (\cref{sec:spectral-gap-selection}):
  ...
  (\cref{rem:fisher-universality}) provide a rigorous structural
...
(\cref{cor:lorentzian-perturbation}) provide a rigorous structural
explanation for why sparse observers perceive exactly one time
dimension.
...
The spectral gap mechanism selects Lorentzian signature but is
agnostic about spatial dimensionality: tests on $d$-dimensional
lattices ($d = 1$--$4$) show smooth variation with~$d$ without
singling out $d = 3 + 1$.
...
selection problem'' is less severe than previously thought:
sparse observers robustly select Lorentzian signature through
the information geometry of their internal interactions, and
multiple independent sign assignment strategies converge to $q = 1$.
