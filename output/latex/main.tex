...
implying that sparse observer graphs select Lorentzian signature ($q = 1$)
with infinite spectral gap margin over all other signatures.
Numerical verification across 199~Ising Fisher matrices confirms
$100\%$ Lorentzian preference for sparse topologies.
...
\item The Tree Fisher Identity shows that the Ising Fisher matrix on
  tree graphs equals $\operatorname{sech}^2(J) \cdot I_m$, and a spectral
  gap selection mechanism demonstrates that sparse observer graphs
  overwhelmingly prefer Lorentzian ($q = 1$) signature
  (\cref{sec:spectral-gap-selection}).
\end{enumerate}
...
for 131~Ising Fisher matrices across five topology classes.
...
Overall: $92\%$ Lorentzian preference.}
...
\item \textbf{Physical origin of edge signs} (HIGH, partially addressed).
  ...
  causal mechanism---sparsity alone selects $q = 1$ with $90\%$--$100\%$
  success rate (\cref{tab:spectral-gap}).
...
\item \textbf{CI and exponential family statistics} (HIGH, partially
  ...
  observers).
  However, CI combined with the maximum entropy principle (Jaynes 1957)
  ...
  with system size, analogous to quantum typicality (Popescu, Short
  \& Winter 2006).
