...
\item \textbf{Physical origin of edge signs} (HIGH).
  ...
  All four fail to produce $q = 1$ for the same structural reason:
  the causal graph provides a binary classification (related vs.\
  unrelated) that generically assigns $q = m$ (all edges timelike),
  not the required $q = 1$.
  The force-fluctuation sign rule yields 23\% Lorentzian success rate.
  A coarse-grained causal direction approach gives $q = 1$ by
  construction but reduces to the force-fluctuation framework.
  Overall confidence that edge signs can be derived from CI alone:
  30\%.
