...
If the internal hyperedge configurations track the \emph{mean parameters}
($w_e(\theta) = \mathbb{E}_\theta[T_e] = \partial A / \partial\theta^e$
for each sufficient statistic component), then
\begin{equation}
...
\begin{theorem}[Perfect Good Regulator $\Rightarrow$ $M \propto F$]
\label{thm:SRC}
Let $O$ be a perfect Good Regulator~\cite{ConantAshby1970,Virgo2025}
satisfying the SRC (\cref{def:SRC}), with the internal hyperedges in
...
An upper bound follows from $\beta_c \leq \|M^-\| / \lambda_{\min}(F)
< \infty$.
...
$\#\{i : d_i < -\beta\}$.
In particular, $g$ has Lorentzian signature (exactly one negative
eigenvalue) for $\beta \in (\beta_{c,2},\, \beta_c)$, where
$\beta_{c,2} = -d_2$ (the second most negative eigenvalue of~$A$,
if it exists with $d_2 < 0$).
For $\beta \in (0,\, \beta_{c,2})$, the metric has two or more
...
For part~(c): Vanchurin's construction has $n$ free sign choices (one
per eigenvector of~$C$).
H1$'$ has $|E_O|$ sign choices, each constrained to produce rank-1
contributions.
When $|E_O| > n$, H1$'$ has more parameters but a more constrained
structure (sum of signed rank-1 terms).
When $|E_O| < n$, H1$'$ has fewer free parameters.
The set of achievable metrics differs in either case.
Explicit counterexamples can be constructed for $n = 2$, $|E_O| = 3$.
\end{proof}
