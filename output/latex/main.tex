...
\item \textbf{CI and exponential family statistics} (HIGH, partially
  ...
  decreases as $O(1/N)$---is supported for global observers
  (KL $\sim 1/N$ in toy models) but \emph{falsified} for local
  observers without global constraint knowledge (typicality fraction
  drops to $0\%$ for $N \geq 10$).
  The strong form of causal typicality is therefore likely false
  ($\sim 25\%$ confidence); a weaker version conditioning on global
