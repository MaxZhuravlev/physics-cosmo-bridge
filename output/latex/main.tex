...
and the effective constant satisfies
\begin{equation}
\label{eq:C-derived}
C(\Delta, J) = \frac{2(\Delta - 1)\operatorname{sech}^2(J)}{(1 + \tanh^g(J))^2}\,,
\end{equation}
derived from the Dobrushin uniqueness condition
(interdependence coefficient $\tanh(J)$, valid for
$\Delta \cdot \tanh(J) < 1$).
...
\max_e \sum_{f \neq e} |F_{ef}|$.
Each edge $e$ has at most $2(\Delta - 1)$ adjacent edges, giving
the dominant contribution $2(\Delta - 1) \operatorname{sech}^4(J)
\cdot \tanh^{g-2}(J) / (1 + \tanh^g(J))^2$.
Division by $\|\operatorname{diag}(F)\|_{\text{op}}
\geq \operatorname{sech}^2(J) / (1 + \tanh^g(J))^2$
yields~\eqref{eq:near-diagonal-bound} with~\eqref{eq:C-derived}.
