...
\item \textbf{Physical origin of edge signs} (HIGH).
  ...
  Deriving it from the causal structure of the embedding hypergraph remains
  the most important open problem.
  Numerical testing of the force-fluctuation sign rule yields 23\% Lorentzian
  success rate compared to 65\% for the adjacency rule, suggesting this
  approach may not be viable in its current form.
...
\item \textbf{CI as constraint on learning dynamics} (SPECULATIVE, largely exhausted).
  Causal invariance constrains reparameterization invariance, uniquely
  selecting natural gradient dynamics.
  Investigation of further constraints (loss function symmetries, landscape
  topology, mass tensor structure) yielded negative results via counterexamples.
  The dataset-learning duality conjecture remains speculative (confidence: 40\%)
  but appears less promising than initially conjectured.
\end{enumerate}
