...
\begin{proposition}[Cycle correction]
\label{prop:cycle-correction}
For an Ising model on a graph with girth~$g$ and uniform coupling~$J$,
the off-diagonal entries of the Fisher matrix satisfy
$|F_{ab}| \leq C\, \tanh^g(J)$ for some constant~$C > 0$.
In particular, sparse graphs with long cycles have near-diagonal Fisher
matrices, and the spectral gap analysis of
...
(\cref{thm:tree-fisher,prop:cycle-correction}).
...
\item \textbf{CI and exponential family statistics} (HIGH, newly
  identified).
  The FSF factorization (\cref{thm:FSF}) and $M = F^2$
  (\cref{thm:M-F-squared}) hold specifically for canonical exponential
  families.
  Whether causal invariance constrains observer statistics to be
  exponential families would close the deductive chain: CI
  $\to$ exponential family $\to$ $M = F^2$ $\to$ FSF $\to$
  Lorentzian.
...
\item \textbf{Extension beyond two parameters} (LOW, numerically confirmed).
  ...
  with robust single-timelike-direction emergence across chain (96.7\%), star (99.4\%),
  complete (99.1\%), random (99.0\%), and tree (97.8\%) topologies.
